...
server = Server(project_path="./", imports=imports) # server seems to like crashing
model = Model()
...
with open(output_path, "w", encoding="utf-8") as out_file:
    for i, theorem in enumerate(tqdm(theorems, desc="Solving theorems")):
        try:
            ...
            final_nps = search_agent.search(initial_sketch=theorem, max_steps=10, verbose=True)
