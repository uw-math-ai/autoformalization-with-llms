...
class AStarSearchAgent():
    ...
    def get_successors(self, nps, k_tries=3, initial_sketch=None, model=None):
        successors = []

        for _ in range(k_tries):
            if model=="Attila":
                current_tactic = None
                if len(nps.prev_tactics) > 0:
                    current_tactic = nps.prev_tactics[-1]
                tactics = self.model.generate_tactics(nps.state, current_tactic, initial_sketch)
            else:
    ...
    def search(self, initial_sketch, max_steps, verbose, which_model=None):
        ...
        while len(p_queue) > 0 and steps < max_steps:
            ...
            successors = self.get_successors(nps, initial_sketch=initial_sketch, model="Attila")
